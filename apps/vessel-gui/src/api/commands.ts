...
export interface VpnStatus {
  status: "disconnected" | "connecting" | "connected" | "disconnecting" | "reconnecting" | "error";
...
export async function vpnProfiles(): Promise<VpnProfile[]> {
  return invoke<VpnProfile[]>("vpn_profiles");
}

export async function vpnAddVless(name: string, uri: string): Promise<VpnProfile> {
  return invoke<VpnProfile>("vpn_add_vless", { name, uri });
}

export async function vpnAddAmnezia(name: string, configText: string): Promise<VpnProfile> {
  return invoke<VpnProfile>("vpn_add_amnezia", { name, configText });
}

export async function vpnRemoveProfile(id: string): Promise<void> {
  return invoke("vpn_remove_profile", { id });
}

export async function vpnConnect(id: string): Promise<void> {
  return invoke("vpn_connect", { id });
}
...
export async function vpnLogs(tail?: number): Promise<string[]> {
  return invoke<string[]>("vpn_logs", { tail: tail ?? null });
}
