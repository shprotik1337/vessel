...
## Что нового в 1.3.17
- **Исправлено зависание GUI без VPN:** Устранено намертво вешающее приложение зависание (deadlock) при попытке включить трек, если Discord заблокирован (из-за синхронного Windows IPC). Теперь Discord RPC работает в изолированном потоке и не блокирует интерфейс.
- **Стабильный серверный прокси:** Исправлен 422 Unprocessable Entity для YouTube Music на сервере. При стриминге треков YouTube Music через Vessel Server аудио корректно проксируется, даже если у потока нет расширения файла (Symphonia распознает по MIME).
...
Скачай установщик `Vessel_1.3.9_x64-setup.exe` из [Releases](https://github.com/shprotik1337/vessel/releases/latest) и запусти — NSIS-инсталлятор (~14 МБ) сделает всё сам. Инсталлятор включает всё нужное: поиск, плеер, импорт плейлистов и лайков.
...
Готовый installer появится в `target/release/bundle/nsis/Vessel_1.3.9_x64-setup.exe`.
