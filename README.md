...
- **встроенный VPN** (Настройки → VPN): профили VLESS (Reality, TLS, WS/gRPC/HTTP) и AmneziaWG — трафик Vessel идёт через встроенное ядро amnezia-box, остальная система не затрагивается, права администратора не нужны;
...
## Vessel Server (экспериментально)

Vessel умеет гибридный режим: каждый провайдер исполняется **локально или на
своем Vessel Server** — тем же кодом провайдеров, только на своей машине.
Зачем: гео-выход (например, googlevideo/POT живут на IP сервера и ретреют
аудио транзитом — без хранения файлов), аккаунт/подписка на одном узле для
всех клиентов, стабильная сеть у дата-центра.

Сервер — **не CDN и не музыкальная база**: постоянного кэша на нём нет, relay
токены живут ограниченное время, временные расшифровки не сохраняются.
Приложение полностью работает и без сервера.

```bash
# сборка бинарника (Linux):
cargo build --release -p vessel-server
# на сервере:
cp deploy/vessel-server.example.toml /opt/vessel/server.toml   # вписать токены
# credentials провайдеров сервер хранит в /opt/vessel/secrets.json (или keyring)
/opt/vessel/bin/vessel-server /opt/vessel/server.toml
# systemd unit:            deploy/vessel-server.service
```

В клиенте: **Настройки → Сервер** — добавить экземпляр (имя, `https://...`,
токен → мгновенное рукопожатие `/api/v1/capabilities`) и выбрать для каждого
...
Альфа-границы: user storage/аккаунты сервера — фаза B; без TLS — ставь
caddy/nginx перед токеном; при гео-блокировках DNS на клиенте
